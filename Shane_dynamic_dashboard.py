...
# ================== DATA + RISK/REWARD FUNCTIONS (same as before) ==================
...
def get_risk_reward_table(ticker, current_price, cfg=None, vol_ratio=1.0, heat_score=0):
    # (Same function as previous version - keeping it short)
    if cfg is None:
        try:
...
# ================== UI (All Tickers Risk/Reward) ==================
st.subheader("🔥 Core Watchlist")
core_df = get_live_data(CORE_WATCHLIST)
st.dataframe(
    core_df.style.map(lambda x: "background-color: #90EE90" if "BREAKOUT" in str(x) else "", subset=["Status"]),
...
if disc_dict:
    disc_df = get_live_data({tk: {"heat_score": v["heat_score"]} for tk, v in disc_dict.items()})
    disc_df = disc_df.sort_values("Heat", ascending=False)
    st.dataframe(disc_df, use_container_width=True, hide_index=True)

# Show Risk/Reward for ALL tickers
...
st.caption("Telegram alerts are now active • Dashboard refreshes automatically")